    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        onBucketUseListener = new PixelPicView.OnPixelTouchListener() {
            ...
            void loop(int x, int y) {
                pic.set(x, y, pen_color);
                if (pic.get(x + 1, y) == original_color)
                    loop(x + 1, y);
                if (pic.get(x - 1, y) == original_color)
                    loop(x - 1, y);
                if (pic.get(x, y + 1) == original_color)
                    loop(x, y + 1);
                if (pic.get(x, y - 1) == original_color)
                    loop(x, y - 1);
            }
            ...
            pixel_size_0 = pic.getScaleX() * dip2px(300) / pic.getWidthPixels();
            {
                ...
                try {
                    ...
                    if (new Date().after(date)) {
                        ...
                        builder.setTitle("警告");
                        builder.setMessage("测试版本已过期，请等接下来的其他测试");
                        builder.setNeutralButton("确定", (dialogInterface, i) -> {
                            dialogInterface.dismiss();
                            finish();
                        });
                    } else {
                        Snackbar.make(rootLayout, R.string.tip1, Snackbar.LENGTH_SHORT).show();
                    }
